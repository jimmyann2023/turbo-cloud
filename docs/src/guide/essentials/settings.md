...
```bash [.env]
# 应用标题
VITE_APP_TITLE=Cloud System

# 应用命名空间，用于缓存、store等功能的前缀，确保隔离
VITE_APP_NAMESPACE=cloud-sysyem
```
...
# 接口地址
VITE_GLOB_API_URL=https://mock-napi.cloud/api
...
```ts
window._CLOUD_SYSTEM_APP_CONF_ = {
  VITE_GLOB_API_URL: 'https://mock-napi.cloud/api',
};
Object.freeze(window._CLOUD_SYSTEM_APP_CONF_);
Object.defineProperty(window, '_CLOUD_SYSTEM_APP_CONF_', {
  configurable: false,
  writable: false,
});
```
...
想要获取 `_app.config.js` 内的变量，需要使用`@repo/hooks`提供的 `useAppConfig`方法。
...
- 首先在 `.env` 或者对应的开发环境配置文件内，新增需要可动态配置的变量，需要以 `VITE_GLOB_*` 开头的变量，如：
  ...
  ```bash
  VITE_GLOB_OTHER_API_URL=https://mock-napi.cloud/other-api
  ```

- 在 `packages/types/global.d.ts`,新增对应的类型定义，如：

  ```ts
  export interface CloudSystemAppConfigRaw {
...
const defaultPreferences: Preferences = {
  ...
  copyright: {
    companyName: 'Cloud',
    companySiteLink: 'https://www.xxx.xxx',
    date: '2024',
    enable: true,
    icp: '',
    icpLink: '',
